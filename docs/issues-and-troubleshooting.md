...
- 実際の OpenAI API エラーメッセージは隠蔽されていたが、最新版では表示されるように修正済み

**確認すべきポイント**:
- OpenAI API キーに `gpt-image-2` の利用権限があるか
- 組織設定で画像生成モデルが有効になっているか
- プロンプトがコンテンツポリシーに違反していないか
- ネットワーク・タイムアウトになっていないか

**次のステップ**:
1. 最新コードで画像生成を実行
2. 画面に表示される具体的なエラーメッセージを確認
3. エラーメッセージに応じてモデル変更（`dall-e-3` など）またはプロンプト修正
